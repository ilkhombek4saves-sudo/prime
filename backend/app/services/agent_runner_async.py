"""
AgentRunnerAsync — async iterative tool-calling loop.

Non-blocking version using httpx.AsyncClient.
"""
...
class AgentRunnerAsync:
    """Async runner — non-blocking tool-calling loop."""
    ...
    async def run(
        ...
        *,
        ...
        result = await self.run_with_meta(
            user_message,
    ...
    async def _run_openai(
        ...
        *,
        ...
        # Stream for plain chat mode
        if on_token and not use_tools:
            text, usage = await self._run_openai_stream(
                model=model,
                max_tokens=max_tokens,
                messages=messages,
                headers=headers,
        ...
        try:
            async with client.stream(
                ...
                if response.status_code >= 400:
                    ...
                    raise ProviderError(f"OpenAI API {response.status_code}: {error_text}")
    ...
    async def _run_anthropic(
        ...
        *,
        ...
        if on_token and not use_tools:
            text, usage = await self._run_anthropic_stream(
                model=model,
                max_tokens=max_tokens,
                messages=messages,
                system=system,
...
_agent_runner_async: AgentRunnerAsync | None = None


def get_agent_runner_async() -> AgentRunnerAsync:
    global _agent_runner_async
    if _agent_runner_async is None:
        _agent_runner_async = AgentRunnerAsync()
    return _agent_runner_async
