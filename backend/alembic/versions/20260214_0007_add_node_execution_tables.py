...
def upgrade() -> None:
    ...
    if "node_approval_queue" not in existing_tables:
        ...
        existing_tables.add("node_approval_queue")

    # Create indexes for node_approval_queue
    if not _index_exists("node_approval_queue", "ix_node_approval_queue_status"):
        op.create_index("ix_node_approval_queue_status", "node_approval_queue", ["status", "created_at"])
    if not _index_exists("node_approval_queue", "ix_node_approval_queue_expires"):
        op.create_index("ix_node_approval_queue_expires", "node_approval_queue", ["expires_at", "status"])
    if not _index_exists("node_approval_queue", "ix_node_approval_queue_connection"):
...
def downgrade() -> None:
    ...
    # Drop node_approval_queue
    if "node_approval_queue" in existing_tables:
        if _index_exists("node_approval_queue", "ix_node_approval_queue_status"):
            op.drop_index("ix_node_approval_queue_status", "node_approval_queue")
        if _index_exists("node_approval_queue", "ix_node_approval_queue_expires"):
            op.drop_index("ix_node_approval_queue_expires", "node_approval_queue")
        if _index_exists("node_approval_queue", "ix_node_approval_queue_connection"):
            op.drop_index("ix_node_approval_queue_connection", "node_approval_queue")
        op.drop_table("node_approval_queue")

    # Drop node_executions
    if "node_executions" in existing_tables:
        if _index_exists("node_executions", "ix_node_executions_status"):
            op.drop_index("ix_node_executions_status", "node_executions")
        if _index_exists("node_executions", "ix_node_executions_connection"):
            op.drop_index("ix_node_executions_connection", "node_executions")
        if _index_exists("node_executions", "ix_node_executions_idempotency"):
            op.drop_index("ix_node_executions_idempotency", "node_executions")
